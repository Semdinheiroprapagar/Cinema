...
export async function POST(request: Request) {
    ...
    try {
      ...
      INSERT INTO posts (title, slug, content, excerpt, cover_image, category, published)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);

        const info = stmt.run(title, slug, content, excerpt, cover_image, category, published ? 1 : 0);
